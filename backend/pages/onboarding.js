...
export default function Onboarding() {
  ...
  const [caredForName, setCaredForName] = useState('Callum')
  const [account, setAccount] = useState({ name: 'Elena Fernández', email: 'elena@example.com', phone: '+34 600 456 789', password: 'heatsafe2026' })
  const [personal, setPersonal] = useState({ name: 'Callum', dob: '1952-03-15', address: 'Calle Sierpes 12, 41004 Sevilla, España', floor: '4', south_facing: true, lives_alone: true })
  const [conditions, setConditions] = useState([])
  const [medications, setMedications] = useState([{ drug_class: 'beta_blockers', label: 'Beta blockers', multiplier: 1.2 }])
  const [selectedMed, setSelectedMed] = useState('')
  const [emergency, setEmergency] = useState({ enabled: true, name: 'Elena Fernández', phone: '+34 600 456 789', relationship: 'Daughter' })
